...
- [ ] Initialize the Next.js App Router project in the root directory
- [ ] Configure custom styles matching the AntiGravity Design Framework (colors: Dark Forest Green, Soft Cream, Light Lime; fonts: Plus Jakarta Sans; rounded-3xl)
- [ ] Create the `.env.local` configuration for Supabase credentials

## Milestone 4: Frontend UI Scaffolding & Components
- [ ] Develop the single-page layout structure (Hero, Services Grid, Meet the Doctor, Testimonials)
- [ ] Build the interactive **Teeth Knowledge Hub** ("Oral Health 101" SVG anatomy, Myth vs. Fact toggles, tips)
- [ ] Implement the Google Maps embed and Floating WhatsApp CTA
- [ ] Design the Local Booking Popup Modal

## Milestone 5: Backend & Hidden Admin Portal
- [ ] Integrate Supabase Client SDK for appointment bookings
- [ ] Implement the **Secret Trigger** (double-click logo) to show the login modal/admin view
- [ ] Develop the Admin Dashboard (appointment table, WhatsApp click-to-contact, CSV export)
- [ ] Add RLS rules & test security constraints
